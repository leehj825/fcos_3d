...
default_load_checkpoint = 'save_state_kitti_keypoint_18.bin'
#default_load_checkpoint = 'save_state_waymo_hpc_29.bin'
#default_load_checkpoint = None

default_output_image_path = 'output_kitti_keypoint_18'
...
def inference(model, image_path, dataset_name):
    ...
    with torch.no_grad():
        ...
        if predictions:
            ...
            keypoint = pred['keypoint']
            depth = pred['depth']
        else:
            boxes, scores, labels, dimensions, orientation, keypoint, depth = [], [], [], [], [], [], []
    return boxes, scores, labels, dimensions, orientation, keypoint, depth
...
def save_combined_image(dataset_name, calib_data, boxes, scores, labels, dimensions, orientation, keypoints, depth, image_path, output_image_path):
    ...
    for box, score, label, dimension, orientation, keypoint, depth in zip(boxes, scores, labels, dimensions, orientation, keypoints, depth):
        if score > 0.5:  # Threshold can be adjusted
            ...
            #Fix: location = torch.cat((offset, depth))
            #print("location", location)

            #corners_3d = create_3d_bbox(dimension, location, orientation)
            #print("bbox", box)
            #print("keypoint", keypoint)
            # Assuming keypoint is a list of 16-element arrays
            # To convert an individual flattened array back to 8x2:
            reshaped_keypoint = keypoint.reshape(8, 2)
            print("reshaped_keypoint", reshaped_keypoint)
            # Convert 2D points to homogeneous 3D points by adding a column of ones
            pts_3d_hom = np.hstack((reshaped_keypoint, np.ones((reshaped_keypoint.shape[0], 1))))

            # Now project to 2D using the calibration matrix P
            corners_2d = project_to_image(pts_3d_hom, P)

            #corners_2d = project_to_image(reshaped_keypoint.T, P)
            print("corners_2d", corners_2d)
    ...
    if mode == 'inference':
        ...
        for batch_idx, (images, targets, calib_data, image_paths) in enumerate(data_loader):
            ...
            for idx, image in enumerate(images):
                ...
                boxes, scores, labels, dimensions, orientation, keypoint, depth = detections

                #print("detections", detections)
                #print("orientation", detections)
                original_image_path = image_paths[0]
                save_combined_image(dataset_name, calib_data[idx], boxes, scores, labels, dimensions, orientation, keypoint, depth, original_image_path, f"{default_output_image_path}/output_{batch_idx}.png")
